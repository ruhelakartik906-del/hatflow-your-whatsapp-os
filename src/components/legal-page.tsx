...
export function LegalPage({ title, updated = "September 24, 2026", children }: { title: string; updated?: string; children: React.ReactNode }) {
  return <main className="legal-page"><article><span>HATFLOW</span><h1>{title}</h1><p className="legal-updated">Last updated: {updated}</p>{children}</article><SiteFooter/></main>;
}