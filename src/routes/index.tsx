...
function Index() {
  ...
  return <main className="overflow-hidden bg-background text-foreground">
    ...
    <section className="logic-section section" id="product"><SectionTitle eyebrow="THE PRODUCT LOGIC" title={<>One Conversation.<br/><span className="gradient-text">Every Business Action.</span></>} copy="Hatflow connects your WhatsApp conversations with the systems that run your business." />
      <div className="logic-map"><div className="logic-main">{([[MessageCircle,"Customer message"],[Zap,"Hatflow receives"],[Bot,"AI understands"],[Contact,"CRM updates"],[GitBranch,"Automation triggers"]] as const).map(([Icon,label],i)=><div className="logic-step" key={label}><div><Icon size={21}/></div><span>{label}</span>{i<4&&<ArrowRight/>}</div>)}</div>
        <div className="logic-branches">{([[ShoppingBag,"Show Product"],[PackageCheck,"Track Order"],[Target,"Create CRM Lead"],[CreditCard,"Confirm Payment"],[RefreshCw,"Recover Cart"],[UsersRound,"Assign Human"]] as const).map(([Icon,label])=><div key={label}><span><Icon size={17}/></span>{label}</div>)}</div>
      </div>
    </section>
