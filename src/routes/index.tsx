...
function Index(){
  const [menuOpen,setMenuOpen]=useState(false); const [annual,setAnnual]=useState(false); const plans=annual?planData.yearly:planData.monthly;
  return <main className="overflow-hidden bg-background text-foreground">
    <header className="site-nav"><a className="nav-brand" href="#top" aria-label="HATFLOW home"><Logo/><span>by UnknownHat Agency</span></a><nav className="desktop-nav">{[["Product","product"],["Solutions","solutions"],["Integrations","integrations"],["Automations","automations"],["Pricing","pricing"],["Resources","faq"]].map(([label,id])=><a key={id} href={`#${id}`}>{label}</a>)}</nav><div className="nav-actions"><a href="#contact" className="login-link">Login</a><Button asChild size="sm"><a href="#pricing">Get Started <ArrowRight/></a></Button></div><Button variant="ghost" size="icon" className="menu-button" aria-label="Toggle menu" onClick={()=>setMenuOpen(!menuOpen)}>{menuOpen?<X/>:<Menu/>}</Button>{menuOpen&&<div className="mobile-nav">{[["Product","product"],["Solutions","solutions"],["Integrations","integrations"],["Automations","automations"],["Pricing","pricing"],["Resources","faq"]].map(([label,id])=><a key={id} href={`#${id}`} onClick={()=>setMenuOpen(false)}>{label}</a>)}</div>}</header>

    <section className="hero" id="top"><div className="hero-grid"/><div className="hero-copy"><div className="brand-kicker"><MessageCircle/>WHATSAPP BUSINESS AUTOMATION PLATFORM</div><h1>Turn WhatsApp Into<br/>Your <span>Business Operating System.</span></h1><p>Connect WhatsApp, manage conversations, capture leads, automate customer journeys and connect your business tools — all from one platform.</p><div className="hero-actions"><Button asChild size="lg"><a href="#pricing">Get Started <ArrowRight/></a></Button><Button asChild variant="outline" size="lg"><a href="#contact">Talk to an Expert</a></Button></div><div className="trust-row">{["Official WhatsApp API","No-Code Automation","AI-Powered Conversations","Built-in CRM","Team Inbox"].map(x=><span key={x}><Check/>{x}</span>)}</div><a className="powered-by" href="https://unknowhat.com" target="_blank" rel="noreferrer"><img src={unknownHatLogo.url} alt="UnknownHat Agency"/><span>Powered by <b>UnknownHat Agency</b></span></a></div><DashboardMockup/></section>

    <section className="logic-section section" id="product"><SectionTitle eyebrow="HOW HATFLOW WORKS" title={<>One Message In.<br/><span className="gradient-text">The Right Business Action Out.</span></>} copy="Hatflow connects every customer message to the right automation, system and next step."/><div className="logic-map"><div className="logic-main">{([[MessageCircle,"Customer Message"],[Zap,"Hatflow"],[Bot,"AI / Automation"],[Braces,"CRM / Integration"],[Rocket,"Business Action"]] as const).map(([Icon,label],i)=><div className="logic-step" key={label}><div><Icon size={21}/></div><span>{label}</span>{i<4&&<ArrowRight/>}</div>)}</div><div className="logic-branches">{([[Target,"Create Lead"],[ShoppingBag,"Send Product"],[PackageCheck,"Track Order"],[RefreshCw,"Recover Cart"],[Clock3,"Schedule Follow-up"],[UsersRound,"Assign Agent"]] as const).map(([Icon,label])=><div key={label}><span><Icon size={17}/></span>{label}</div>)}</div></div></section>
    ...
    <footer><div className="footer-main"><div className="footer-brand"><Logo light/><p>WhatsApp Business. Automated.</p><span>A product by <a href="https://unknowhat.com" target="_blank" rel="noreferrer">UnknownHat Agency</a></span></div><nav className="footer-legal" aria-label="Support and legal"><a href="/support">Support</a><a href="/privacy-policy">Privacy Policy</a><a href="/terms-and-conditions">Terms &amp; Conditions</a><a href="/disclaimer">Disclaimer</a><a href="/refund-return-policy">Refund &amp; Return Policy</a></nav></div><div className="footer-bottom"><span>© 2026 Hatflow. All rights reserved.</span></div></footer>
