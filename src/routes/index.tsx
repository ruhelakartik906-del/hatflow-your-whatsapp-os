...
function Index(){
  ...
  return <main className="overflow-hidden bg-background text-foreground">
    ...
    <section className="section faq-section" id="faq"><SectionTitle eyebrow="FAQ" title="Frequently Asked Questions" copy="Straight answers about setup, integrations, pricing and how Hatflow works."/><div className="faq-list">{faqs.map(([q,a],i)=><details className="reveal" key={q} style={{["--rd" as string]:`${Math.min(i*0.04,0.3)}s`}}><summary>{q}<ChevronDown/></summary><p>{a}</p></details>)}</div></section>
